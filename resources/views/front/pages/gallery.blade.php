...
<div class="flex-1">
    <section class="relative w-full aspect-[21/9] min-h-[400px] flex items-center justify-center overflow-hidden">
        ...
        <div class="relative z-20 text-center px-6 max-w-4xl">
            ...
            <h1 class="text-white text-4xl md:text-6xl font-black leading-tight mb-6">{{ $gallery_hero->items->where('field_key', 'title')->first()->field_value ??'' }}</h1>
            <p class="text-white/80 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed">
                {{ $gallery_hero->items->where('field_key', 'description')->first()->field_value ??'' }}
            </p>
        </div>
    </section>
    <div class="max-w-7xl mx-auto px-6 py-16 space-y-24">
        <section>
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
                <div>
                    <h2 class="text-primary text-3xl font-black uppercase tracking-tight mb-2">{{ $gallery_factory->items->where('field_key', 'factory_facilities_title')->first()->field_value ??'' }}</h2>
                    <p class="text-slate-500 max-w-lg">{{ $gallery_factory->items->where('field_key', 'factory_facilities_subtitle')->first()->field_value ??'' }}</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-2 group relative overflow-hidden rounded-xl">
                    ...
                    <div
                        ...
                        <h4 class="text-white font-bold text-lg">{{$gallery_factory->items->where('field_key','factory_facility_1_section')->first()->field_value ?? ''}}</h4>
                        <p class="text-white/80 text-sm">{{ $gallery_factory->items->where('field_key','factory_facility_1_description')->first()->field_value ?? '' }}</p>
                    </div>
                </div>
                <div class="flex flex-col gap-6">
                    <div class="group relative overflow-hidden rounded-xl h-full">
                        ...
                        <div
                            class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                            <h4 class="text-white font-bold text-lg">{{$gallery_factory->items->where('field_key','factory_facility_2_section')->first()->field_value ?? ''}}</h4>
                            <p class="text-white/80 text-sm">{{ $gallery_factory->items->where('field_key','factory_facility_2_description')->first()->field_value ?? '' }}</p>
                        </div>
                    </div>
                    <div class="group relative overflow-hidden rounded-xl h-full">
                        <div class="h-full min-h-[200px] bg-center bg-cover transition-transform duration-700 group-hover:scale-110"
                        ...
                        <div
                            class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                            <h4 class="text-white font-bold text-lg">{{$gallery_factory->items->where('field_key','factory_facility_3_section')->first()->field_value ?? ''}}</h4>
                            <p class="text-white/80 text-sm">{{ $gallery_factory->items->where('field_key','factory_facility_3_description')->first()->field_value ?? '' }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section class="bg-primary/5 -mx-6 px-6 py-16 rounded-3xl">
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
                <div>
                    <h2 class="text-primary text-3xl font-black uppercase tracking-tight mb-2">{{ $gallery_production->items->where('field_key','production_process_title')->first()->field_value ?? '' }}</h2>
                    <p class="text-slate-500 max-w-lg">{{ $gallery_production->items->where('field_key','production_process_subtitle')->first()->field_value ?? '' }}</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="space-y-4">
                    ...
                    <div>
                        <h5 class="font-bold text-slate-800">{{ $gallery_production->items->where('field_key', 'production_process_1_section')->first()->field_value ?? 'Fiber Preparation' }}</h5>
                        <p class="text-sm text-slate-500">{{ $gallery_production->items->where('field_key', 'production_process_1_description')->first()->field_value ?? '' }}</p>
                    </div>
                </div>
                <div class="space-y-4">
                    <div class="relative rounded-xl overflow-hidden aspect-square shadow-lg">
                    ...
                    <div>
                        <h5 class="font-bold text-slate-800">{{ $gallery_production->items->where('field_key', 'production_process_2_section')->first()->field_value ?? 'Mechanical Molding' }}</h5>
                        <p class="text-sm text-slate-500">{{ $gallery_production->items->where('field_key', 'production_process_2_description')->first()->field_value ?? '' }}</p>
                    </div>
                </div>
                <div class="space-y-4">
                    <div class="relative rounded-xl overflow-hidden aspect-square shadow-lg">
                    ...
                    <div>
                        <h5 class="font-bold text-slate-800">{{ $gallery_production->items->where('field_key', 'production_process_3_section')->first()->field_value ?? 'High-Temp Drying' }}</h5>
                        <p class="text-sm text-slate-500">{{ $gallery_production->items->where('field_key', 'production_process_3_description')->first()->field_value ?? '' }}</p>
                    </div>
                </div>
                <div class="space-y-4">
                    <div class="relative rounded-xl overflow-hidden aspect-square shadow-lg">
                    ...
                    <div>
                        <h5 class="font-bold text-slate-800">{{ $gallery_production->items->where('field_key', 'production_process_4_section')->first()->field_value ?? 'Quality Inspection' }}</h5>
                        <p class="text-sm text-slate-500">{{ $gallery_production->items->where('field_key', 'production_process_4_description')->first()->field_value ?? '' }}</p>
                    </div>
                </div>
            </div>
        </section>
        <section>
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
                <div>
                    <h2 class="text-primary text-3xl font-black uppercase tracking-tight mb-2">{{ $gallery_packaging->items->where('field_key', 'packaging_title')->first()->field_value ?? '' }}</h2>
                    <p class="text-slate-500 max-w-lg">{{ $gallery_packaging->items->where('field_key', 'packaging_subtitle')->first()->field_value ?? '' }}</p>
                </div>
                <div class="h-1 flex-grow mx-8 bg-primary/10 mb-4 hidden md:block"></div>
                ...
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-primary/5 group">
                    <div class="relative rounded-xl overflow-hidden mb-4">
                        ...
                        <div
                            ...
                            {{ $gallery_packaging->items->where('field_key', 'packaging_1_section')->first()->field_value ?? '' }}</div>
                    </div>
                    <p class="text-slate-700">
                        {{ $gallery_packaging->items->where('field_key', 'packaging_1_description')->first()->field_value ?? '' }}
                    </p>
                </div>
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-primary/5 group">
                    <div class="relative rounded-xl overflow-hidden mb-4">
                        ...
                        <div
                            class="absolute bottom-3 right-3 bg-white/90 px-3 py-1 rounded-full text-xs font-bold text-primary">
                            {{ $gallery_packaging->items->where('field_key', 'packaging_2_section')->first()->field_value ?? '' }}</div>
                    </div>
                    <p class="text-slate-700">
                        {{ $gallery_packaging->items->where('field_key', 'packaging_2_description')->first()->field_value ?? '' }}
                    </p>
                </div>
            </div>
        </section>
         <section class="bg-primary/5 -mx-6 px-6 py-16 rounded-3xl">
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
                <div>
                    <h2 class="text-primary text-3xl font-black uppercase tracking-tight mb-2">{{ $gallery_qc->items->where('field_key', 'quality_control_title')->first()->field_value ?? '' }}</h2>
                    <p class="text-slate-500 max-w-lg">{{ $gallery_qc->items->where('field_key', 'quality_control_subtitle')->first()->field_value ?? '' }}</p>
                </div>
                <div class="h-1 flex-grow mx-8 bg-primary/20 mb-4 hidden md:block"></div>
                <span class="material-symbols-outlined text-4xl text-primary/20">repeat</span>
            </div>
             <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-primary/5 group">
                    ...
                    <p class="text-slate-700">
                        {{ $gallery_qc->items->where('field_key', 'quality_1_description')->first()->field_value ?? '' }}
                    </p>
                </div>
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-primary/5 group">
                    <div class="relative rounded-xl overflow-hidden mb-4">
                    ...
                    <p class="text-slate-700">
                        {{ $gallery_qc->items->where('field_key', 'quality_2_description')->first()->field_value ?? '' }}
                    </p>
                </div>
                    </p>
                </div>
